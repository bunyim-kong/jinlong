...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
            <h3 class="text-lg font-semibold text-gray-800">Edit Property</h3>
            <p class="text-sm text-gray-500 mt-1">Update property information</p>
        </div>

        <form action="{{ route('properties.update', $property->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="address" rows="3" required
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">{{ old('address', $property->address) }}</textarea>
                    @error('address')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="total_unit" value="{{ old('total_unit', $property->total_unit) }}" required min="1"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">
                    @error('total_unit')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
